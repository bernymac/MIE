...
void* 
paillier_ciphertext_to_bytes( int len, paillier_ciphertext_t* ct, void* bytes )
{
//	void* buf;
	int cur_len;
    ...
    if (len != cur_len)
        perror("paillier_ciphertext_to_bytes wrong length provided");
//	buf = malloc(len);
	memset(bytes, 0, len);
//	mpz_export(bytes + (len - cur_len), 0, 1, 1, 0, 0, ct->c);
    mpz_export(bytes, 0, 1, 1, 0, 0, ct->c);
