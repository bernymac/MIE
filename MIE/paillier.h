...
void* paillier_ciphertext_to_bytes( int len, paillier_ciphertext_t* ct, void* bytes );
...
/*
    Utility method that gives the ciphertext size for a given public key,
    which is a function of its size: ciphertextSize = 2*n. This method is
    helpfull when storing and reading ciphertexts to/from byte arrays.
*/
int paillier_get_ciphertext_size( paillier_pubkey_t* pub );
